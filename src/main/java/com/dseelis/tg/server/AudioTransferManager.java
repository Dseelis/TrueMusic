...
import com.dseelis.tg.network.packets.AudioChunkPacket;
import com.dseelis.tg.network.packets.AudioStreamStartPacket;
...
public class AudioTransferManager {
    ...
    public void queueTransfer(ServerPlayer player, UUID resourceId, long startPositionMs) {
        ...
        if (audioPath == null) {
            TrueMusic.LOGGER.warn("Cannot queue transfer: audio {} not found on server", resourceId);
            return;
        }
        ...
        for (PendingTransfer t : queue) {
            if (t.resourceId.equals(resourceId)) {
                TrueMusic.debugLog("Transfer {} already queued for player {}", resourceId, player.getName().getString());
                return;
            }
        }
        ...
        int totalChunks = (int) Math.ceil((double) dataSize / TrueMusicServerConfig.getChunkSize());
        ...
        TrueMusic.LOGGER.info("Queued transfer {} for player {} ({} chunks, startOffset={}, startPositionMs={})",
    ...
    public void tick(net.minecraft.server.MinecraftServer server) {
        ...
        synchronized (activePlayerOrder) {
            ...
            int maxBytesPerTick = TrueMusicServerConfig.getMaxBytesPerTick();
            int maxBytesPerPlayerPerTick = TrueMusicServerConfig.getMaxBytesPerPlayerPerTick();
            ...
            while (totalBytesSent < maxBytesPerTick && playersProcessed < activePlayerOrder.size() && iterations < maxIterations) {
                ...
                int chunkSize = TrueMusicServerConfig.getChunkSize();
                ...
                if (chunkData != null) {
                    ...
                    if (transfer.nextChunkIndex >= transfer.totalChunks) {
                        queue.poll();
                        TrueMusic.LOGGER.info("Transfer complete: {} to {}", transfer.resourceId, player.getName().getString());
                    ...
                    queue.poll();
                    TrueMusic.LOGGER.error("Failed to read chunk at offset {} for {}", readOffset, transfer.resourceId);
                }
    ...
    private byte[] readChunkAt(Path audioPath, long offset, int maxSize) {
        try (RandomAccessFile raf = new RandomAccessFile(audioPath.toFile(), "r")) {
            ...
            TrueMusic.LOGGER.error("Failed to read audio chunk", e);
            return null;
        }
    }
